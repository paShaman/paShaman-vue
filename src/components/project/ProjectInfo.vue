        ...
        <div class="w-full md:w-2/3 text-left">
            <Fancybox v-if="project.image">
                <a :href="project.host + (project.image_full || project.image)" data-fancybox>
                    <img :src="project.host + project.image" class="rounded-xl cursor-pointer shadow-lg sm:shadow-none">
                </a>
            </Fancybox>
            ...
            <template v-if="project.image">
                <div class="mb-7">
                    <ul class="leading-loose grid gap-3">
                        <li v-for="author in project.authors" class="font-normal text-ternary-dark dark:text-ternary-light flex items-center gap-5">
                ...
                <div class="mb-7">
                    ...
                    <div class="font-normal text-primary-dark dark:text-ternary-light flex gap-1 flex-wrap">
                        <router-link to="/" @click="globalTag.tag = tag" v-for="tag in project.tags" class="inline-block rounded-md px-2.5 border-2 dark:border-white/20 hover:border-gray-400 cursor-pointer">
                            {{ tag }}
                        </router-link>
                    </div>
                </div>
                ...
                <div class="mb-7" v-if="project.years && project.years.length > 1">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <a :href="year.link"
                           ...
                           class="font-medium flex items-center px-4 py-2 rounded-lg shadow-lg hover:shadow-xl text-white duration-300 cursor-pointer"
                           :class="{'bg-indigo-500 hover:bg-indigo-600': !year.current, 'bg-emerald-500 hover:bg-emerald-600': year.current}"
                        >
                            {{ year.year }}
                        </a>
                    </div>
                ...
                <div class="mb-7 px-6 py-4 rounded-xl bg-gradient-to-r" v-if="project.works && project.works.length > 1" :class="gradientClass">
                    ...
                    <div class="grid gap-4 text-primary-dark">
                        <div v-for="work in project.works" class="flex gap-2 items-start">
                            <a :href="work.link" target="_blank" class="font-medium hover:underline cursor-pointer w-[6rem]">
                                {{ work.name }}
                            </a>
